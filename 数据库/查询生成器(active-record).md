...
用 AR 而不是原生的 SQL 语句去执行数据库查询，可以调用直观方法来实现相同目标。如，调用 [[Zilf\Db\ActiveRecord::save()|save()]] 方法将执行插入或更新轮询，将在该 AR 类关联的数据表新建或更新一行数据：
...
* MySQL 4.1 及以上：通过 [[Zilf\Db\ActiveRecord]]
* PostgreSQL 7.3 及以上：通过 [[Zilf\Db\ActiveRecord]]
* SQLite 2 和 3：通过 [[Zilf\Db\ActiveRecord]]
* Microsoft SQL Server 2010 及以上：通过 [[Zilf\Db\ActiveRecord]]
* Oracle: 通过 [[Zilf\Db\ActiveRecord]]
* CUBRID 9.1 及以上：通过 [[Zilf\Db\ActiveRecord]]
...
要想声明一个 AR 类，你需要扩展 [[Zilf\Db\ActiveRecord]] 基类，
...
AR 用一个 [[Zilf\Db\Connection|DB connection]] 对象与数据库交换数据。
...
，你可以覆盖掉 [[Zilf\Db\ActiveRecord::getDb()|getDb()]] 方法：
...
AR 提供了两种方法来构建 DB 查询并向 AR 实例里填充数据：
 ...
 - [[Zilf\Db\ActiveRecord::find()]]
 - [[Zilf\Db\ActiveRecord::findBySql()]]

以上两个方法都会返回 [[Zilf\Db\ActiveQuery]] 实例，该类继承自[[Zilf\Db\Query]]，
...
- [[Zilf\Db\ActiveRecord::save()|save()]]
- [[Zilf\Db\ActiveRecord::insert()|insert()]]
- [[Zilf\Db\ActiveRecord::update()|update()]]
- [[Zilf\Db\ActiveRecord::delete()|delete()]]

AR 同时提供了一下静态方法，可以应用在与某 AR 类所关联的整张表上。
用这些方法的时候千万要小心，因为他们作用于整张表！
比如，`deleteAll()`  会删除掉表里**所有**的记录。

- [[Zilf\Db\ActiveRecord::updateCounters()|updateCounters()]]
- [[Zilf\Db\ActiveRecord::updateAll()|updateAll()]]
- [[Zilf\Db\ActiveRecord::updateAllCounters()|updateAllCounters()]]
- [[Zilf\Db\ActiveRecord::deleteAll()|deleteAll()]]
...
> 用哪个取决于当前 AR 对象是不是新对象（在函数内部，他会检查 [[Zilf\Db\ActiveRecord::isNewRecord]] 的值）。
...
1. constructor
2. [[Zilf\Db\ActiveRecord::init()|init()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_INIT|EVENT_INIT]] 事件

当你通过 [[Zilf\Db\ActiveRecord::find()|find()]] 方法查询数据时，每个AR实例都将有以下生命周期：

1. constructor
2. [[Zilf\Db\ActiveRecord::init()|init()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_INIT|EVENT_INIT]] 事件
3. [[Zilf\Db\ActiveRecord::afterFind()|afterFind()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_AFTER_FIND|EVENT_AFTER_FIND]] 事件

当通过 [[Zilf\Db\ActiveRecord::save()|save()]] 方法写入或者更新数据时, 我们将获得如下生命周期：

1. [[Zilf\Db\ActiveRecord::beforeValidate()|beforeValidate()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_BEFORE_VALIDATE|EVENT_BEFORE_VALIDATE]] 事件
2. [[Zilf\Db\ActiveRecord::afterValidate()|afterValidate()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_AFTER_VALIDATE|EVENT_AFTER_VALIDATE]] 事件
3. [[Zilf\Db\ActiveRecord::beforeSave()|beforeSave()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_BEFORE_INSERT|EVENT_BEFORE_INSERT]] 或 [[Zilf\Db\ActiveRecord::EVENT_BEFORE_UPDATE|EVENT_BEFORE_UPDATE]] 事件
4. 执行实际的数据写入或更新
5. [[Zilf\Db\ActiveRecord::afterSave()|afterSave()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_AFTER_INSERT|EVENT_AFTER_INSERT]] 或 [[Zilf\Db\ActiveRecord::EVENT_AFTER_UPDATE|EVENT_AFTER_UPDATE]] 事件

最后，当调用 [[Zilf\Db\ActiveRecord::delete()|delete()]] 删除数据时, 我们将获得如下生命周期：

1. [[Zilf\Db\ActiveRecord::beforeDelete()|beforeDelete()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_BEFORE_DELETE|EVENT_BEFORE_DELETE]] 事件
2. 执行实际的数据删除
3. [[Zilf\Db\ActiveRecord::afterDelete()|afterDelete()]]: 会触发一个 [[Zilf\Db\ActiveRecord::EVENT_AFTER_DELETE|EVENT_AFTER_DELETE]] 事件
...
定义关联关系使用一个可以返回 [[Zilf\Db\ActiveQuery]] 对象的 getter 方法，
[[Zilf\Db\ActiveQuery]]对象有关联上下文的相关信息，因此可以只查询关联数据。
...
class Order extends \Zilf\Db\ActiveRecord
{
...
以上使用了 [[Zilf\Db\ActiveRecord::hasMany()]] 和 [[Zilf\Db\ActiveRecord::hasOne()]] 方法。
以上两例分别是关联数据多对一关系和一对一关系的建模范例。
如，一个客户有很多订单，一个订单只归属一个客户。
两个方法都有两个参数并返回 [[Zilf\Db\ActiveQuery]] 对象。
...
`hasMany()` 返回 [[Zilf\Db\ActiveQuery]] 对象，该对象允许你通过
[[Zilf\Db\ActiveQuery]] 方法定制查询。
...
> Note: 关联查询返回的是 [[Zilf\Db\ActiveQuery]] 的实例，如果像特性（如类属性）那样连接关联数据，
返回的结果是关联查询的结果，即 [[Zilf\Db\ActiveRecord]] 的实例，
...
有时，两个表通过中间表关联，定义这样的关联关系， 可以通过调用 [[Zilf\Db\ActiveQuery::via()|via()]] 方法或 [[Zilf\Db\ActiveQuery::viaTable()|viaTable()]] 方法来定制 [[Zilf\Db\ActiveQuery]] 对象 。
...
两个方法是相似的，除了 [[Zilf\Db\ActiveQuery::via()|via()]] 方法的第一个参数是使用 AR 类中定义的关联名。 以上方法取代了中间表，等价于：
...
为解决以上性能问题，可以通过调用 [[Zilf\Db\ActiveQuery::with()]] 方法使用即时加载解决。
...
为避免多余执行的后一条语句，我们可以为 customer或 orders 关联关系定义相反的关联关系，通过调用 [[Zilf\Db\ActiveQuery::inverseOf()|inverseOf()]] 方法可以实现。
...
> Note: 相对关系不能在包含中间表的关联关系中定义。 即是，如果你的关系是通过[[Zilf\Db\ActiveQuery::via()|via()]] 或 [[Zilf\Db\ActiveQuery::viaTable()|viaTable()]]方法定义的， 就不能调用[[Zilf\Db\ActiveQuery::inverseOf()]]方法了。
...
JOIN SQL语句的查询条件和参数，使用 [[Zilf\Db\ActiveQuery::joinWith()]]
可以重用已定义关系并调用
而不是使用 [[Zilf\Db\ActiveQuery::join()]] 来实现目标。
...
以上，方法 [[Zilf\Db\ActiveQuery::innerJoinWith()|innerJoinWith()]] 是访问 `INNER JOIN` 类型的  [[Zilf\Db\ActiveQuery::joinWith()|joinWith()]] 的快捷方式。
...
[[Zilf\Db\ActiveQuery::joinWith()|joinWith()]] 和  [[Zilf\Db\ActiveQuery::with()|with()]] 的区别是
...
当使用 [[Zilf\Db\ActiveQuery::joinWith()|joinWith()]] 方法时可以响应没有歧义的列名。
...
默认 [[Zilf\Db\ActiveQuery::joinWith()|joinWith()]] 使用左连接来连接关联表。
你也可以传 `$joinType` 参数来定制连接类型。
你也可以使用 [[Zilf\Db\ActiveQuery::innerJoinWith()|innerJoinWith()]]。
...
这可以通过调用 [[Zilf\Db\ActiveQuery::onCondition()]] 方法实现：
...
在上面， [[Zilf\Db\ActiveRecord::hasMany()|hasMany()]] 方法回传了一个 [[Zilf\Db\ActiveQuery]] 对象，
当你用 [[Zilf\Db\ActiveQuery::joinWith()|joinWith()]] 执行一条查询时，取决于正被调用的是哪个 [[Zilf\Db\ActiveQuery::onCondition()|onCondition()]]，
返回 `category_id` 为 1 的 items

当你用 [[Zilf\Db\ActiveQuery::joinWith()|joinWith()]] 进行一次查询时，“on-condition”条件会被放置在相应查询语句的 ON 部分，
如：
...
注意：如果通过 [[Zilf\Db\ActiveQuery::with()]] 进行贪婪加载或使用惰性加载的话，则 on 条件会被放置在对应 SQL语句的 `WHERE` 部分。
...
* [[Zilf\Db\ActiveRecord::link()|link()]]
* [[Zilf\Db\ActiveRecord::unlink()|unlink()]]
...
[[Zilf\Db\ActiveRecord::link()|link()]] 调用上述将设置 customer_id 的顺序是 $customer 的主键值，然后调用 [[Zilf\Db\ActiveRecord::save()|save()]] 要将顺序保存到数据库中。


作用域
--------------

当你调用[[Zilf\Db\ActiveRecord::find()|find()]] 或 [[Zilf\Db\ActiveRecord::findBySql()|findBySql()]]方法时，将会返回一个[[Zilf\Db\ActiveQuery|ActiveQuery]]实例。之后，你可以调用其他查询方法，如 [[Zilf\Db\ActiveQuery::where()|where()]]，[[Zilf\Db\ActiveQuery::orderBy()|orderBy()]], 进一步的指定查询条件。
...
1. 类必须继承 Zilf\Db\ActiveQuery (或者是其他的 ActiveQuery ，比如 yii\mongodb\ActiveQuery)。
2. 必须是一个public类型的方法且必须返回 $this 实现链式操作。可以传入参数。
3. 检查 [[Zilf\Db\ActiveQuery]] 对于修改查询条件是非常有用的方法。

其次，覆盖[[Zilf\Db\ActiveRecord::find()]] 方法使其返回自定义的查询对象而不是常规的[[Zilf\Db\ActiveQuery|ActiveQuery]]。对于上述例子，你需要编写如下代码：
...
```php
class Post extends \Zilf\Db\ActiveRecord
{
...
如果你之前用过 Yii 1.1 就应该知道默认作用域的概念。一个默认的作用域可以作用于所有查询。你可以很容易的通过重写[[Zilf\Db\ActiveRecord::find()]]方法来定义一个默认作用域，例如：
...
注意，你之后所有的查询都不能用 [[Zilf\Db\ActiveQuery::where()|where()]]，但是可以用 [[Zilf\Db\ActiveQuery::andWhere()|andWhere()]] 和 [[Zilf\Db\ActiveQuery::orWhere()|orWhere()]]，他们不会覆盖掉默认作用域。（译注：如果你要使用默认作用域，就不能在 xxx::find()后使用where()方法，你必须使用andXXX()或者orXXX()系的方法，否则默认作用域不会起效果，至于原因，打开where()方法的代码一看便知）
...
, [[Zilf\Db\ActiveRecord::afterSave()|afterSave()]], [[Zilf\Db\ActiveRecord::beforeDelete()|beforeDelete()]] and/or [[Zilf\Db\ActiveRecord::afterDelete()|afterDelete()]] 生命周期周期方法(life cycle methods 我觉得这句翻译成“模板方法”会不会更好点？)。开发者可以通过重写[[Zilf\Db\ActiveRecord::save()|save()]]方法然后在控制器里使用事务操作，严格地说是似乎不是一个好的做法 （召回"瘦控制器 / 肥模型"基本规则）。

这些方法在这里(如果你不明白自己实际在干什么，请不要使用他们)，Models：

```php
class Feature extends \Zilf\Db\ActiveRecord
{
    // ...
...
class Product extends \Zilf\Db\ActiveRecord
{
    // ...
...
重写 [[Zilf\Db\ActiveRecord::save()|save()]] 方法：

```php
class ProductController extends \Zilf\System\Controller
{
...
被污染属性
-------------------
当你调用[[Zilf\Db\ActiveRecord::save()|save()]]用于保存活动记录(Active Record)实例时,只有被污染的属性才会被保存。一个属性是否认定为被污染取决于它的值自从最后一次从数据库加载或者最近一次保存到数据库后到现在是否被修改过。注意:无论活动记录(Active Record)是否有被污染属性，数据验证始终会执行。

活动记录(Active Record)会自动维护一个污染数据列表。它的工作方式是通过维护一个较旧属性值版本，并且将它们与最新的进行比较。你可以通过调用[[Zilf\Db\ActiveRecord::getDirtyAttributes()]]来获取当前的污染属性。你也可以调用[[Zilf\Db\ActiveRecord::markAttributeDirty()]]来显示的标记一个属性为污染属性。

如果你对最近一次修改前的属性值感兴趣，你可以调用[[Zilf\Db\ActiveRecord::getOldAttributes()|getOldAttributes()]] 或 [[Zilf\Db\ActiveRecord::getOldAttribute()|getOldAttribute()]]。

另见
-------------------

- [模型（Model）](model.md)
- [[Zilf\Db\ActiveRecord]]
